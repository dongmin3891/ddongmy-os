...
가독성을 위해 query 정의를 사용하는 컴포넌트에서 분리하되, 해당 feature 가까이에
둔다. 옵션을 추상화하기 전에 데이터 소유자와 cache 정책을 먼저 드러낸다.
...
2. transport와 응답 schema를 정의한다.
3. query option과 key 계층을 정의한다.
4. 화면의 pending, error, empty, success 상태를 구현한다.
5. mutation과 cache 갱신 범위를 구현한다.
6. 중복 요청, 취소, focus refetch, network 재연결과 hydration을 검증한다.
